...
abstract class BaseClass(
    ...
    // В ChildrenClass это поле публичное
    private val privateVal: String,
    // 2. объясни, почему это поле недоступно в main()
    // Потому что поле protected
    ...
    private var privateField = "6. добавь сеттер чтобы изменить меня из main()" // private поле нельзя изменять в дочернем классе
    fun getAll(): String {
        return mapOf(
    ...
    //  вызывается текст из дочернего метода
...
fun main() {
    val child = ChildrenClass(
        privateVal = "privateTest",
        protectedVal = "protectedTest",
        publicVal = "publicTest"
    )
    child.publicField = "Антонио Бандерас"
    child.updateProtectedField("Значение изменено")
    print(child.getAll())
